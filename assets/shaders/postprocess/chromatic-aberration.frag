...
void main(){
    ...
    float green = texture(tex, tex_coord)[1];

    vec2 newCoord = tex_coord - vec2(STRENGTH, 0);
    float red = texture(tex, newCoord)[0];

    newCoord = tex_coord + vec2(STRENGTH, 0);
    float blue = texture(tex,newCoord)[2];
