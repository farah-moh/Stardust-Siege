...
void main(){
    ...
    // Multiply by 2 to scale from 0 to 2 then minus 1 to be from -1 to 1
    vec2 ndcCoord = tex_coord*2 - 1;

    // dot(ndcCoord, ndcCoord) gets the length squared
    frag_color = texture(tex, tex_coord)/(1.0 + dot(ndcCoord,ndcCoord));    
}